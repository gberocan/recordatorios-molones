...
// Añadimos listeners para parar la alarma si el usuario clica o presiona cualquier tecla
document.addEventListener('click', pararAlarma);
document.addEventListener('keydown', pararAlarma);
...
function mostrarTareas() {
    ...
    tareas.forEach(t => {
        ...
        // Mejor mover esta función fuera del bucle para no redeclararla cada vez
...
// Definimos esta función fuera del forEach para no repetirla
...
function inicioAlertaTitulo(nombreTarea, hora) {
  ...
  // Detener después de 5 minutos (300000 ms)
  timeoutDetener = setTimeout(() => {
    detenerAlertaTitulo();
  }, 300000);

  // Añadir event listeners para clic y tecla
  window.addEventListener('click', detenerAlertaTitulo);
  window.addEventListener('keydown', detenerAlertaTitulo);
}
...
function detenerAlertaTitulo() {
  ...
  if (timeoutDetener) {
    clearTimeout(timeoutDetener);
    timeoutDetener = null;
  }
  document.title = tituloOriginal;

  // Remover event listeners para no acumularlos
  window.removeEventListener('click', detenerAlertaTitulo);
  window.removeEventListener('keydown', detenerAlertaTitulo);
}
    ...
    tareas.forEach(t => {
        if (!t.hecha && !t.alertada) {
            ...
            if (Math.abs(momentoTarea - ahora) < 30000) {
                ...
                // NUEVO: Mostrar notificación flotante
                mostrarNotificacionTarea(t);
            }
        }
    });
}, 5000);
...
const notificacion = document.getElementById('notificacion-tarea');

function mostrarNotificacionTarea(tarea) {
    const fechaFormateada = formatearFecha(tarea.fecha);
    notificacion.innerText = `Es hora de: ${tarea.texto}!\n\n\n(Pulsa cualquer tecla para parar la alarma)`;
    notificacion.classList.add('visible');

    setTimeout(() => {
        notificacion.classList.remove('visible');
    }, 30000); // Se oculta automáticamente después de 30 segundos
}
